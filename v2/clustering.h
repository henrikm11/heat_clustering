...
/// @brief computed the expected value of dataInd.second
/// @param dataInd {i,data[i]} with i original index
/// @return expected value
double expectedValue(const std::vector<std::pair<int,double>>& dataInd);
   
/// @brief computed the variance of dataInd.second
/// @param dataInd {i,data[i]} with i original index
/// @param expectedValue expected value of dataInd.second
/// @return variance
double variance(const std::vector<std::pair<int,double>>& dataInd, double expectedValue);
double variance(const std::vector<std::pair<int,double>>& dataInd);
...
class kNN : public Graph{
public:
    kNN(const std::vector<std::vector<double>>& data, int k);
    kNN(const Graph&, int k);

    //~kNN(void); probably destructor of base is sufficient?
...
private:
    Graph* getPrekNN(const std::vector<double>& data, int k);
    Graph* getkNNHelper(Graph* G, int k);
    Graph* getkNN(const std::vector<double>& data, int k);
...
//easy to do the below getting a copied subgraph using constructor on vector of vertices now
//Graph* getSubGraph(std::vector<Node*>);