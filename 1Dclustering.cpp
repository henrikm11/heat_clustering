...
//wait until heat in initial point is cooled off to (factor)*(equilibrium amount)
...
std::unordered_map<double,int>* oneDimensionalClustering(std::vector<double>& data, double significance, double bandWidth, double minClusterSize){
    ...
    //approximate density via windows of size bandWidth
    //if maxDensity>=significance*minDensity we label cluster
